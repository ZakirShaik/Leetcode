...
public:
   int mySqrt(int x) {
       ...
       while(start <= end) {
           long long mid = (start + end)/2;  // Here (start + end)/2 causes integer overflow if int is used as datatype. So, long long is being used. It's better to use start + (end - start)/2 to use int
